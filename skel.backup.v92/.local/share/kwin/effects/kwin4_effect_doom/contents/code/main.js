...
class BurnMyWindowsDoomEffect {
  constructor() {
    ...
    this.shader = effect.addFragmentShader(Effect.MapTexture, 'doom.frag');
  ...
  loadConfig() {
    ...
    // I have not yet found a way to scaling the actor canvas on KWin. So this effect
    // cannot look as good as on GNOME.
    ...
    effect.setUniform(this.shader, 'uVerticalScale',
                      effect.readConfig('VerticalScale', 1.0));
  ...
  slotWindowAdded(window) {
    ...
    if (!BurnMyWindowsDoomEffect.shouldAnimateWindow(window)) {
      return;
    }
    if (!window.visible) {
      return;
